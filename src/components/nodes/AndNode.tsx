...
export default function AndNode({ node, onDragStart, onPortClick, canvasOffset, canvasScale, isSelected }: AndNodeProps) {
  const inputPorts = node.inputPorts;
  const outputPort = node.outputPorts[0];

  // Логика AND: все входы должны быть true
      ...
      <div className={`relative group rounded-2xl transition-all ${
        ...
        >
          ...
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
            isActive ? 'bg-blue-500/30' : 'bg-gray-700/50'
          }`}>
            <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke={isActive ? '#3b82f6' : '#9ca3af'} strokeWidth="2">
              <path d="M7 12h10M7 8h10M7 16h10" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </div>

          <div className="flex flex-col gap-1">
            <span className="text-white font-semibold text-sm tracking-wide">AND</span>
        ...
        {inputPorts.map((port, index) => (
          ...
          >
            <div className={`absolute inset-1 rounded-full ${
              port.value ? 'bg-blue-300' : 'bg-gray-500'
            }`} />
          </div>
        ))}
