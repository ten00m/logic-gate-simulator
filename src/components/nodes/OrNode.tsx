...
export default function OrNode({ node, onDragStart, onPortClick, canvasOffset, canvasScale, isSelected }: OrNodeProps) {
  const inputPorts = node.inputPorts;
  const outputPort = node.outputPorts[0];

  // Логика OR: хотя бы один вход должен быть true
      ...
      <div className={`relative group rounded-2xl transition-all ${
        ...
        <div
          className={`flex items-center gap-3 px-5 py-4 rounded-2xl border-2 transition-all duration-300 backdrop-blur-sm ${
            isActive
              ? 'bg-gradient-to-br from-purple-900/70 to-purple-800/50 border-purple-400 shadow-xl shadow-purple-500/30'
        ...
        >
          ...
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
            ...
            <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke={isActive ? '#a855f7' : '#9ca3af'} strokeWidth="2">
              <path d="M12 5v14M5 12h14" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </div>

          <div className="flex flex-col gap-1">
            <span className="text-white font-semibold text-sm tracking-wide">OR</span>
          ...
          <div className={`w-3 h-3 rounded-full transition-all duration-300 ${
            isActive
              ? 'bg-purple-400 shadow-md shadow-purple-400/50 animate-pulse'
              : 'bg-gray-600'
          }`} />
        </div>
        ...
        {inputPorts.map((port, index) => (
          ...
          >
            <div className={`absolute inset-1 rounded-full ${
              port.value ? 'bg-purple-300' : 'bg-gray-500'
            }`} />
          </div>
        ))}
        ...
        >
          <div className={`absolute inset-1 rounded-full ${
            outputValue ? 'bg-purple-300' : 'bg-gray-500'
          }`} />
        </div>
      </div>
    </div>
