...
export default function NorNode({ node, onDragStart, onPortClick, canvasOffset, canvasScale, isSelected }: NorNodeProps) {
  const inputPorts = node.inputPorts;
  const outputPort = node.outputPorts[0];

  // Логика NOR: NOT OR
  const isActive = !(inputPorts.length > 0 && inputPorts.some(port => port.value));
      ...
      <div className={`relative group rounded-2xl transition-all ${
        ...
        <div
          className={`flex items-center gap-3 px-5 py-4 rounded-2xl border-2 transition-all duration-300 backdrop-blur-sm ${
            isActive
              ? 'bg-gradient-to-br from-pink-900/70 to-pink-800/50 border-pink-400 shadow-xl shadow-pink-500/30'
              : 'bg-gradient-to-br from-gray-800/90 to-gray-900/90 border-gray-600 hover:border-gray-500'
          }`}
        >
          {/* Иконка */}
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
            isActive ? 'bg-pink-500/30' : 'bg-gray-700/50'
          }`}>
            <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke={isActive ? '#ec4899' : '#9ca3af'} strokeWidth="2">
              <path d="M12 5v14M5 12h14" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </div>

          <div className="flex flex-col gap-1">
            <span className="text-white font-semibold text-sm tracking-wide">NOR</span>
          ...
          <div className={`w-3 h-3 rounded-full transition-all duration-300 ${
            isActive
              ? 'bg-pink-400 shadow-md shadow-pink-400/50 animate-pulse'
              : 'bg-gray-600'
          }`} />
        </div>
        ...
        {inputPorts.map((port, index) => (
          <div
            ...
            className={`absolute left-0 w-5 h-5 rounded-full border-3 cursor-pointer transition-all duration-200 hover:scale-125 z-10 ${
              port.value
                ? 'bg-pink-400 border-pink-300 shadow-lg shadow-pink-400/50'
          ...
          >
            <div className={`absolute inset-1 rounded-full ${
              port.value ? 'bg-pink-300' : 'bg-gray-500'
            }`} />
          </div>
        ))}
        ...
        <div
          ...
          className={`absolute right-0 w-5 h-5 rounded-full border-3 cursor-pointer transition-all duration-200 hover:scale-125 z-10 ${
            outputValue
              ? 'bg-pink-400 border-pink-300 shadow-lg shadow-pink-400/50'
              : 'bg-gray-600 border-gray-400 hover:bg-gray-500'
          }`}
        ...
        >
          <div className={`absolute inset-1 rounded-full ${
            outputValue ? 'bg-pink-300' : 'bg-gray-500'
          }`} />
        </div>
      </div>
    </div>
