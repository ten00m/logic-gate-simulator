...
export default function XnorNode({ node, onDragStart, onPortClick, canvasOffset, canvasScale, isSelected }: XnorNodeProps) {
  const inputPorts = node.inputPorts;
  const outputPort = node.outputPorts[0];

  // Логика XNOR: четное количество true входов (включая 0)
  const isActive = inputPorts.filter(port => port.value).length % 2 === 0;
      ...
      <div className={`relative group rounded-2xl transition-all ${
        ...
        <div
          className={`flex items-center gap-3 px-5 py-4 rounded-2xl border-2 transition-all duration-300 backdrop-blur-sm ${
            isActive
              ? 'bg-gradient-to-br from-violet-900/70 to-violet-800/50 border-violet-400 shadow-xl shadow-violet-500/30'
              : 'bg-gradient-to-br from-gray-800/90 to-gray-900/90 border-gray-600 hover:border-gray-500'
          }`}
        >
          {/* Иконка */}
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
            isActive ? 'bg-violet-500/30' : 'bg-gray-700/50'
          }`}>
            <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke={isActive ? '#a78bfa' : '#9ca3af'} strokeWidth="2">
              <path d="M7 12h10M7 8h10M7 16h10" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </div>

          <div className="flex flex-col gap-1">
            <span className="text-white font-semibold text-sm tracking-wide">XNOR</span>
          ...
          <div className={`w-3 h-3 rounded-full transition-all duration-300 ${
            isActive
              ? 'bg-violet-400 shadow-md shadow-violet-400/50 animate-pulse'
              : 'bg-gray-600'
          }`} />
        </div>
        ...
        {inputPorts.map((port, index) => (
          ...
          >
            <div className={`absolute inset-1 rounded-full ${
              port.value ? 'bg-violet-300' : 'bg-gray-500'
            }`} />
          </div>
        ))}
        ...
        >
          <div className={`absolute inset-1 rounded-full ${
            outputValue ? 'bg-violet-300' : 'bg-gray-500'
          }`} />
        </div>
      </div>
    </div>
