...
export default function NandNode({ node, onDragStart, onPortClick, canvasOffset, canvasScale, isSelected }: NandNodeProps) {
  const inputPorts = node.inputPorts;
  const outputPort = node.outputPorts[0];

  // Логика NAND: NOT AND
  const isActive = !(inputPorts.length > 0 && inputPorts.every(port => port.value));
      ...
      <div className={`relative group rounded-2xl transition-all ${
        ...
        <div
          className={`flex items-center gap-3 px-5 py-4 rounded-2xl border-2 transition-all duration-300 backdrop-blur-sm ${
            isActive
              ? 'bg-gradient-to-br from-orange-900/70 to-orange-800/50 border-orange-400 shadow-xl shadow-orange-500/30'
              : 'bg-gradient-to-br from-gray-800/90 to-gray-900/90 border-gray-600 hover:border-gray-500'
          }`}
        >
          {/* Иконка */}
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
            isActive ? 'bg-orange-500/30' : 'bg-gray-700/50'
          }`}>
            <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none" stroke={isActive ? '#f97316' : '#9ca3af'} strokeWidth="2">
              <path d="M7 12h10M7 8h10M7 16h10" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </div>

          <div className="flex flex-col gap-1">
            <span className="text-white font-semibold text-sm tracking-wide">NAND</span>
          ...
          <div className={`w-3 h-3 rounded-full transition-all duration-300 ${
            isActive
              ? 'bg-orange-400 shadow-md shadow-orange-400/50 animate-pulse'
              : 'bg-gray-600'
          }`} />
        </div>
        ...
        {inputPorts.map((port, index) => (
          ...
          >
            <div className={`absolute inset-1 rounded-full ${
              port.value ? 'bg-orange-300' : 'bg-gray-500'
            }`} />
          </div>
        ))}
        ...
        >
          <div className={`absolute inset-1 rounded-full ${
            outputValue ? 'bg-orange-300' : 'bg-gray-500'
          }`} />
        </div>
      </div>
    </div>
